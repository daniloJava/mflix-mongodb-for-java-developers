...
public class MovieDao extends AbstractMFlixDao {
	...
	/**
	 * movieId needs to be a hexadecimal string value. Otherwise it won't be possible to translate to an ObjectID
	 *
	 * @param movieId
	 *                - Movie object identifier
	...
	public Document getMovie(String movieId) {
		...
		Document movie = moviesCollection.aggregate(pipeline).batchSize(1).iterator().tryNext();
		return movie;
	}
	 ...
	 *
	 * @param limit
	 *              - max number of documents to be returned.
	 * @param skip
	 *              - number of documents to be skipped.
	 * @param sort
	 ...
	 *
	 * @param country
	 *                - Country string value to be matched.
	...
	public List<Document> getMoviesByCountry(String... country) {
		...
		moviesCollection.find(queryFilter).projection(new Document("title", 1)).into(movies);

		return movies;
	}

	/**
	 * This method will execute the following mongo shell query: db.movies.find({"$text": { "$search": `keywords` }},
	 * {"score": {"$meta": "textScore"}}).sort({"score": {"$meta": "textScore"}})
	 *
	 * @param limit
	 *                 - integer value of number of documents to be limited to.
	 * @param skip
	 *                 - number of documents to be skipped.
	 * @param keywords
	 *                 - text matching keywords or terms
	...
	public List<Document> getMoviesByText(int limit, int skip, String keywords) {
		...
		List<Document> movies = new ArrayList<>();
		moviesCollection.find(textFilter).projection(projection).sort(sort).skip(skip).limit(limit).iterator().forEachRemaining(movies::add);
		return movies;
	}

	/**
	 * Finds all movies that contain any of the `casts` members, sorted in descending by the `sortKey` field.
	 *
	 * @param sortKey
	 *                - sort key.
	 * @param limit
	...
	/**
	 * This method is the java implementation of the following mongo shell aggregation pipeline { "$bucket": { "groupBy":
	 * "$runtime", "boundaries": [0, 60, 90, 120, 180], "default": "other", "output": { "count": {"$sum": 1} } } }
	 */
	...
	/*
	 * This method is the java implementation of the following mongo shell aggregation pipeline { "$bucket": { "groupBy":
	 * "$metacritic", "boundaries": [0, 50, 70, 90, 100], "default": "other", "output": { "count": {"$sum": 1} } } }
	 */
	...
	/**
	 * This method is the java implementation of the following mongo shell aggregation pipeline pipeline.aggregate([
	 * {$match: {cast: {$in: ... }}}, {$sort: {tomatoes.viewer.numReviews: -1}}, {$skip: ... }, {$limit: ... }, {$facet:{
	 * runtime: {$bucket: ...}, rating: {$bucket: ...}, movies: {$addFields: ...}, }} ])
	 */
	...
	/**
	 * This method is the java implementation of the following mongo shell aggregation pipeline pipeline.aggregate([ ...,
	 * {$facet:{ runtime: {$bucket: ...}, rating: {$bucket: ...}, movies: {$addFields: ...}, }} ])
	 *
	 * @return Bson defining the $facet stage.
	 */
	private Bson buildFacetStage() {

		return Aggregates.facet(new Facet("runtime", buildRuntimeBucketStage()), new Facet("rating", buildRatingBucketStage()),
	 ...
	 *
	 * @param keywords
	 *                 - set of keywords that match the query
	 * @return number of matching documents.
	 */
	public long getTextSearchCount(String keywords) {
		return moviesCollection.countDocuments(Filters.text(keywords));
	}
	...
	public long getCastSearchCount(String... cast) {
		return moviesCollection.countDocuments(Filters.in("cast", cast));
	}
	...
	public long getGenresSearchCount(String... genres) {
		return moviesCollection.countDocuments(Filters.in("genres", genres));
	}
}
