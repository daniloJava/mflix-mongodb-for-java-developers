...
public class UserTest extends TicketTest {
	...
	private static String email = "tokio@lacasadepapel.edu";
	...
	public void setup() {
		...
		dao = new UserDao(mongoClient, databaseName);
		testUser = new User();
		testUser.setName("Tokio");
		testUser.setEmail(email);
		testUser.setHashedpw("somehahedpw");
		jwt = "somemagichejwt";
		mongoClient.getDatabase("mflix").getCollection("users").deleteOne(new Document("email", "log@out.com"));
	}
	...
	public void testRegisterUser() {
		...
		assertTrue("Should have correctly created the user - check your write user method", dao.addUser(testUser)); // add string explanation
	...
	public void testLogin() {
		...
		assertEquals("The user email needs to match the `session` user_id field", testUser.getEmail(), session.getUserId());
	...
	public void testLogout() {
		...
		assertTrue("Should have deleted user from sessions collection - check your logout method", dao.deleteUser(email));
	...
	public void testDeleteUser() {
		dao.addUser(testUser);
		assertTrue("You should be able to delete correctly the testDb user. Check your delete filter", dao.deleteUser(testUser.getEmail()));

		assertNull("Should not find any sessions after deleting a user. deleteUser() method needs to remove the user sessions data!", dao.getUserSession(testUser.getEmail()));

		assertNull("User data should not be found after user been deleted. Make sure you delete data from users collection", dao.getUser(testUser.getEmail()));
	}
}
